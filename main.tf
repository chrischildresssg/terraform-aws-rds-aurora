...
resource "aws_db_subnet_group" "this" {
  count = var.create == true && var.db_subnet_group_name == "" ? 1 : 0
...
resource "aws_rds_cluster_instance" "this" {
  count = var.create ? var.replica_scale_enabled ? var.replica_scale_min : var.replica_count : 0
...
resource "aws_appautoscaling_target" "read_replica_count" {
  count = var.create && var.replica_scale_enabled ? 1 : 0
...
resource "aws_security_group_rule" "default_ingress" {
  count = var.create && var.create_security_group ? length(var.allowed_security_groups) : 0
...
resource "aws_security_group_rule" "cidr_ingress" {
  count = var.create && var.create_security_group && length(var.allowed_cidr_blocks) > 0 ? 1 : 0
